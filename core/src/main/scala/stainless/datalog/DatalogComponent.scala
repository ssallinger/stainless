...
object DatalogComponent extends SimpleComponent {
  ...
  override def apply(funs: Seq[Identifier], p: StainlessProgram, ctx: inox.Context): DatalogAnalysis = {
    ...
    object Embedder {
        //TODO Should we have unique nodes? especially types -- one fact per type would be enough
        //but then problem that we have pointer to parent and not other way around :(
        ...
        def emit(fd: FunDef, parentId: Int, astOrder: Int): Unit = {
          ...
          counter = 1
          for (callee <- p.symbols.callees(fd.id)) {
        ...
        private def emitNode(name: String, id: Int, parentId: Int, astOrder: Int, nodeType: String) {
            ...
            bw.write(s"""Node(${nameStr}, ${id}, ${parentId}, $astOrder, "$nodeType").\n""")
        }
        
        private def emitParameter(name: String, nodeId: Int, parameterOrder: Int, value: String) {
            bw.write(s"""Parameter("${name}", ${nodeId}, $parameterOrder, "${value}").\n""")
        }
        
    }  
    
    
    for (fun <- p.symbols.functions.values) { //we don't restrict to "toEmbed" as we want whole AST
        Embedder.emit(fun, -1, -1)
    }
    
    for (adt <- p.symbols.adts.values) {
        Embedder.emit(adt, -1, -1)    
    }
    
    bw.close()
    
    //return dummy result, ignore for now (we are only interested in embedding which is written to file)
    val dummy: Long = 0
