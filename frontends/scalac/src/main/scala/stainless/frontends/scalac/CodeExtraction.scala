...
trait CodeExtraction extends ASTExtractors {
  ...
  lazy val reporter = ctx.reporter
  ...
  def extractUnit(unit: CompilationUnit): (xt.UnitDef, Seq[xt.ClassDef], Seq[xt.FunDef]) = {
    val (id, stats) = unit.body match {
      // package object
      case PackageDef(refTree, List(pd @ PackageDef(inner, body))) =>
        (getIdentifier(refTree.symbol), pd.stats)

      case pd @ PackageDef(refTree, lst) =>
        (getIdentifier(refTree.symbol), pd.stats)

      case _ => outOfSubsetError(unit.body, "Unexpected unit body")
    }
    ...
    val ud = xt.UnitDef(id, imports, classes, subs, !isLibrary).setPos(unit.body.pos)
    (ud, allClasses.toList, allFunctions.toList)
  }
  ...
  protected def extractStatic(stats: List[Tree]): (
    Seq[xt.Import],
    Seq[Identifier],
    Seq[Identifier],
    Seq[xt.ModuleDef],
